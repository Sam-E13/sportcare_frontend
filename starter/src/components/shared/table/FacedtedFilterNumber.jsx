...
import {
  ...
  export function FacedtedFilterNumber({
    column,
    title,
    options,
    labelField = "label",
    ...
    const selectedItems = options?.filter((o) =>
      selectedValues.includes(Number(o[valueField]))
    );
  ...
  function ComboboxFilterNumber({
    ...
    return (
      <Combobox
        value={options?.filter((o) => selectedValues.includes(Number(o[valueField])))}
        onChange={(list) => {
          column.setFilterValue(list.map((item) => Number(item[valueField])));
        }}
        multiple
                  ...
                  {({ selected }) => (
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className="font-mono text-xs">
                        {facets?.get(Number(item[valueField]))}
                      </span>
                    </div>
                  )}
                </ComboboxOption>
  ...
  FacedtedFilterNumber.propTypes = {
  ...
  ComboboxFilterNumber.propTypes = {
    column: PropTypes.object,
    title: PropTypes.string,
    labelField: PropTypes.string,
    valueField: PropTypes.string,
