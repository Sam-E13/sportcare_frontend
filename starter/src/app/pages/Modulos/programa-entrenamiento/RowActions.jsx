...
import { ProgramaEntrenamientoDrawer } from "./ProgramaEntrenamientoDrawer";
import { useDisclosure } from "hooks";
import { deletePrograma } from "./api/ProgramaEntrenamientoApi";
...
const confirmMessages = {
  pending: {
    description:
      "¿Estás seguro de que deseas eliminar este Programa? Una vez eliminada, no se puede restaurar.",
  },
  success: {
...
export function RowActions({ row, table }) {
  ...
  const handleDeletePrograma = useCallback(async () => {
    setConfirmDeleteLoading(true);
    try {
      await deletePrograma(row.original.id);
  ...
  return (
    <>
      ...
      <ConfirmModal
        ...
        onOk={handleDeletePrograma}
        confirmLoading={confirmDeleteLoading}
        state={state}
      />

      <ProgramaEntrenamientoDrawer row={row} close={closeDrawer} isOpen={isDrawerOpen} />
    </>
  );
}
