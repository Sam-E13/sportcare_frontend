...
export function ConsultorioModal({ isOpen, onClose, consultorio, mode, onSubmit }) {
  ...
  // Actualizar formData cuando cambia el consultorio
  useEffect(() => {
    if (consultorio) {
      setFormData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (result) { // Cambio importante aquí
        onClose();
      } else {
        ...
        >
          <DialogPanel className="relative flex w-full max-w-2xl origin-top flex-col overflow-hidden rounded-lg bg-white transition-all duration-300 dark:bg-dark-700">
            <div className="flex items-center justify-between rounded-t-lg bg-gray-200 px-4 py-3 dark:bg-dark-800 sm:px-5">
              ...
              >
                {mode === 'create' ? 'Nuevo Consultorio' : 'Editar Consultorio'}
              </DialogTitle>
              <Button
                onClick={onClose}
                variant="flat"
