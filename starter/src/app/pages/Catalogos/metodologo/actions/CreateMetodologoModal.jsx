...
import { createMetodologo, getAllGruposDeportivos, getAllDeportes, getAllUsuarios } from "../api/metodologoList.api";
import { toast } from "sonner";

export function CreateMetodologoModal({ isOpen, onClose, onCreate }) {
  const [formData, setFormData] = useState({
    user: "", // Campo de usuario requerido
    nombre: "",
    aPaterno: "",
    aMaterno: "",
    grupos: [], // Array de IDs de grupos seleccionados
        ...
        try {
          ...
          const gruposOpts = [
            ...
            ...gruposResponse.data.map(grupo => ({
              ...
              label: grupo.nombre,
              disabled: false,
              deportes: grupo.deportes || [] // Asumiendo que los grupos tienen relación con deportes
            }))
          ];
          
          // Formatear opciones para deportes (todos los deportes disponibles)
  ...
  useEffect(() => {
    if (formData.grupos.length > 0) {
      console.log("Grupos seleccionados:", formData.grupos);
      console.log("Deportes disponibles:", deportesOptions);
      
      const deportesFiltradosPorGrupo = deportesOptions.filter(deporte => {
        console.log(`Deporte ${deporte.label} pertenece al grupo ${deporte.grupoId}`);
        return formData.grupos.includes(deporte.grupoId);
      });
      
      console.log("Deportes filtrados:", deportesFiltradosPorGrupo);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await createMetodologo({
        user: formData.user, // ID del usuario seleccionado
        nombre: formData.nombre.trim(),
        aPaterno: formData.aPaterno.trim(),
        aMaterno: formData.aMaterno.trim(),
        grupos: formData.grupos, // Array de IDs de grupos
        deportes: formData.deportes // Array de IDs de deportes
      });
      
      onCreate(response.data);
      onClose();
      toast.success("Metodólogo creado correctamente");
      setFormData({
        user: "",
        nombre: "",
        aPaterno: "",
      ...
      console.error("Error al crear metodólogo:", err);
      setError(err.response?.data?.message || "Error al crear metodólogo");
      toast.error("Error al crear metodólogo");
    } finally {
      setIsLoading(false);
    }
  };
