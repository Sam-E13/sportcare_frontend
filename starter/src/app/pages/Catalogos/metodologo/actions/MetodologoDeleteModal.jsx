...
const confirmMessages = {
  pending: {
    ...
    title: "¿Estás seguro?",
    description: (metodologo) => `¿Estás seguro de que deseas eliminar al metodólogo "${metodologo.nombre} ${metodologo.aPaterno}"? Esta acción no se puede deshacer.`,
    actionText: "Eliminar"
  },
  success: {
    Icon: AnimatedTick,
    ...
    title: "¡Eliminado!",
    description: () => "El metodólogo ha sido eliminado correctamente del sistema.",
    actionText: "Hecho"
  },
  error: {
    Icon: XCircleIcon,
    ...
    title: "Error",
    description: (metodologo) => `No se pudo eliminar al metodólogo "${metodologo.nombre} ${metodologo.aPaterno}". Verifica que no esté siendo usado en otros registros.`,
    actionText: "Reintentar"
  }
};

export function MetodologoDeleteModal({ isOpen, onClose, row, onDelete }) {
  ...
  const handleDelete = async () => {
    try {
      ...
      await onDelete(row);
      setStatus("success");
      toast.success(`Metodólogo "${metodologo.nombre} ${metodologo.aPaterno}" eliminado correctamente`);
    } catch (error) {
      console.error("Error deleting metodologo:", error);
      setStatus("error");
      toast.error(`Error al eliminar el metodólogo "${metodologo.nombre} ${metodologo.aPaterno}"`);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          <DialogPanel className="relative flex w-full max-w-md origin-top flex-col overflow-hidden rounded-lg bg-white transition-all duration-300 dark:bg-dark-700">
            ...
            <div className="flex flex-col items-center px-4 py-6 sm:px-5 sm:py-8">
              ...
              <p className="text-center text-gray-800 dark:text-dark-100">
                {currentMessage.description(metodologo)}
              </p>
              {status === "error" && (
