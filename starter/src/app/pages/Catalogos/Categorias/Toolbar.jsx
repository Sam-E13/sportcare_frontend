...
const exportCategoriasToPDF = (table) => {
  try {
    const doc = new jsPDF({
    ...
    const cleanData = table.getCoreRowModel().rows.map(row => {
      ...
      columns.forEach(col => {
        ...
        switch (col.id) {
          case 'edadMin':
          case 'edadMax':
            cleanRow[col.id] = `${cellValue} años`;
            break;
          default:
    ...
    autoTable(doc, {
      ...
      columnStyles: {
        nombre: { cellWidth: 40 },
        edadMin: { cellWidth: 20 },
        edadMax: { cellWidth: 20 },
        rangoEdad: { cellWidth: 25 }
      }
    });
    ...
    doc.save(`Reporte_Categorias_${dateString.replace(/\//g, '-')}.pdf`);
...
export function Toolbar({ table }) {
  ...
  const handleCreateSuccess = (newCategoria, isUpdate) => {
    table.options.meta?.addRow(newCategoria, isUpdate);
  };
          ...
          <Menu as="div" className="relative inline-block text-left">
            ...
            >
              ...
              <MenuItem>
                {({ focus }) => (
                  <button
                    onClick={() => exportCategoriasToPDF(table)}
                    className={clsx(
                      "flex h-9 w-full items-center space-x-3 px-3 tracking-wide outline-none transition-colors rtl:space-x-reverse",
                      focus && "bg-gray-100 text-gray-800 dark:bg-dark-600 dark:text-dark-100"
                    )}
