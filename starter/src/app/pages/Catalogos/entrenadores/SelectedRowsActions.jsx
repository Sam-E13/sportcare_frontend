...
import { deleteEntrenador } from "./api/entrenadorList.api";
...
export function SelectedRowsActions({ table }) {
  ...
  const handleDeleteRows = async () => {
    if (selectedRows.length > 0) {
      ...
      try {
        // Eliminar cada entrenador individualmente
        await Promise.all(
          selectedRows.map(row => deleteEntrenador(row.original.id))
        );
        // Actualizar la tabla
  ...
  const handleExportPDF = () => {
    try {
      ...
      doc.save(`entrenadores_${new Date().toISOString().slice(0,10)}.pdf`);
